...
class PostgresqlOneConfig extends BaseConfig
{
    const key = "postgresql";
    /**
     * @var string
     */
    protected $name;
    ...
    public function buildConfig()
    {
        if (!extension_loaded('pdo_pgsql')) {
            throw new PostgresqlException("缺少postgresql扩展");
        }
        if ($this->poolMaxNumber < 1) {
            throw new PostgresqlException("poolMaxNumber必须大于1");
        }
        if(empty($this->name)){
            throw new PostgresqlException("name必须设置");
        }
        if(empty($this->host)){
            throw new PostgresqlException("host必须设置");
        }
        if(empty($this->username)){
            throw new PostgresqlException("username必须设置");
        }
        if(empty($this->password)){
            throw new PostgresqlException("password必须设置");
        }
        if(empty($this->db)){
            throw new PostgresqlException("db必须设置");
        }
