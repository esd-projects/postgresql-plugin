...
abstract class GoModel implements TracingInterface
{
    ...
    {
        $this->_roles = [
            [static::getPrimaryKey(), "required", "on" => "update"]
        ];
        $this->_translates = [];
        $this->_messages = [];
    ...
    public function insert($selectDb = "default")
    {
        ...
        $data = $this->buildToArray(false);
        $id = $this->postgresql($selectDb)
            ->insert($this->getTableName(), $data);
        if ($id === false) {
            throw new PostgresqlException($this->postgresql($selectDb)->getLastError(), $this->postgresql($selectDb)->getLastErrno());
        }
        $pk = $this->getPrimaryKey();
        $this->$pk = $id;
    }
